...
class AblationCAMNEW(BaseCAM):
    def __init__(self,
        ...
        super(AblationCAMNEW, self).__init__(model,
                                          target_layers,
                                          use_cuda,
                                          reshape_transform,
    ...
    def get_cam_weights(self,
        ...
        with torch.no_grad():
            ...
            for batch_index, (target, tensor) in enumerate(
                ...
                for i in range(
                    ...
                    output_raw = [o for o in self.model(batch_tensor)]
                    score = [target(o).cpu().item() for o in output_raw]
                    score_ = [self.get_scores(o, target).cpu().item() for o in output_raw]
                    scores_.extend(score_)
        ...
        return weights * np.array(scores_).reshape(activations.shape[:2])
        # weights = weights * np.array(scores_).reshape(activations.shape[:2])
        # return torch.nn.functional.normalize(torch.from_numpy(weights).cuda(), p=1, dim=-1).cpu().numpy()
