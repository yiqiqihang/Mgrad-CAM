    ...
    def get_grad_list(self, target_all_list, target_all_index):
        ...
        for item in range(len(target_all_list)):
            ...
            for grad in grads:
                ...
                # grad_weights = self.min_max_normalization_tensor(grad)
                grad_weights = torch.nn.functional.normalize(grad, p=1, dim=0)
                grad_ = grad_weights[target_all_index[item], ...] * grad[target_all_index[item], ...] + grad[target_all_index[item], ...]
