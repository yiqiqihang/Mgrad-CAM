...
def get_args():
    ...
    parser.add_argument('--savepath', type=str, default="/devdata/home/homefun/weights/CAM_MobileNetV2")
...
def get_model():
    # ---------- 修改点 1: 更换为 MobileNetV2 ----------
    model = models.mobilenet_v2(weights=None)
    # MobileNetV2 的分类头是一个 Sequential，最后一个线性层在 index 1，输入特征维度是 1280
...
if __name__ == '__main__':
    ...
    resume = os.path.join(args.path, r"weights/coco_cam/loss_mobile20260527151208/best_acc.pth")
    ...
    # ---------- 修改点 2: 替换 CAM 的 Target Layers 为 MobileNetV2 结构 ----------
    # MobileNetV2 的特征提取层包含在 model.features (一个 Sequential) 中。
    # 为了匹配你的 Mgrad-CAM 中采用的多层递归融合策略 (Re-FML，对应 ResNet50 的 layer4, layer3, layer2)
    # 我们需要找到感受野和特征分辨率对应 MobileNetV2 的模块索引：
    # features[-1] (Index 18): 分辨率 1/32 (对应 ResNet layer4)
    # features[13]: 分辨率 1/16 (对应 ResNet layer3)
    # features[6]: 分辨率 1/8 (对应 ResNet layer2)
    if layer == 432:
        target_layers = [model.features[6], model.features[13], model.features[-1]]
    elif layer == 43:
        target_layers = [model.features[13], model.features[-1]]
    elif layer == 42:
        target_layers = [model.features[6], model.features[-1]]
    elif layer == 32:
        target_layers = [model.features[6], model.features[13]]
    else:
